...
class SqliteAdminUserStore {
  ...
  async listAdmins(tenantId) {
    const rows = this.db.prepare(`
      ...
      ORDER BY email
    `).all(requireTenantId(tenantId || 'default'));
  ...
  async listMembersWithRoles(tenantId) {
    ...
    `).all(requireTenantId(tenantId || 'default')).map(row => ({
      ...row,
  ...
  async addAdmin(email, tenantId, role) {
    return this.addMembership(email, tenantId || 'default', role || 'admin');
  }
  ...
  async removeAdmin(email, tenantId) {
    return this.removeMembership(email, tenantId || 'default');
  }
}
